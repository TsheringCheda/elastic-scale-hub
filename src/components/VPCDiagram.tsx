  ...
  return (
    <Card className="p-6 shadow-aws">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="font-bold text-lg text-foreground">Custom VPC Architecture</h3>
          <p className="text-sm text-muted-foreground">Network topology overview</p>
        </div>
      </div>
      ...
      <div className="relative p-6 bg-gradient-to-br from-accent/5 to-secondary/5 rounded-lg border-2 border-dashed border-accent/30">
        ...
        <div className="flex justify-center mb-6">
          <div className="flex items-center gap-2 p-3 bg-secondary/20 rounded-lg border border-secondary/50">
            ...
            <span className="text-sm font-medium text-secondary">Internet Gateway</span>
          </div>
        </div>
        ...
        <div className="flex justify-center mb-6">
          <div className="p-4 bg-primary/10 rounded-lg border-2 border-primary/30 shadow-aws">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-primary">Application Load Balancer</span>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {['us-east-1a', 'us-east-1b'].map((az, index) => (
            <div key={az} className="p-4 bg-muted/30 rounded-lg border border-muted">
              <div className="flex items-center gap-2 mb-3">
                <Shield className="h-4 w-4 text-muted-foreground" />
                <span className="text-sm font-medium text-foreground">{az}</span>
              </div>
              
              {/* Public Subnet */}
              <div className="mb-3 p-2 bg-success/10 rounded border border-success/30">
                <div className="text-xs font-medium text-success mb-1">Public Subnet</div>
                <div className="text-xs text-muted-foreground">
                  10.0.{index + 1}.0/24
                </div>
              </div>
              ...
              <div className="p-2 bg-warning/10 rounded border border-warning/30">
                <div className="text-xs font-medium text-warning mb-2">Private Subnet</div>
                ...
                <div className="p-2 bg-card rounded border border-border">
                  <div className="flex items-center gap-1 mb-1">
                    ...
                    <span className="text-xs font-medium">i-{Math.random().toString(36).substr(2, 9)}</span>
                  </div>
                  <div className="text-xs text-muted-foreground">
                    HTTP Server Running
                  </div>
            ...
            <div key={nat} className="flex justify-center">
              <div className="p-2 bg-secondary/10 rounded border border-secondary/30">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs text-secondary">NAT Gateway {nat}</span>
                </div>
              </div>
            </div>
          ))}
