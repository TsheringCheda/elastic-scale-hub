    ...
    const interval = setInterval(() => {
      ...
      setInstances((prev) =>
        prev.map((instance) => ({
          ...
          cpuUsage: parseFloat(
            Math.max(
              20,
              Math.min(90, instance.cpuUsage + (Math.random() - 0.5) * 10)
            ).toFixed(2)
          ),
        }))
      );
      ...
      setLoadBalancer((prev) => ({
        ...
        requestsPerMinute: parseFloat(
          Math.max(
            100,
            Math.min(500, prev.requestsPerMinute + (Math.random() - 0.5) * 50)
          ).toFixed(2)
        ),
        responseTime: parseFloat(
          Math.max(
            15,
            Math.min(80, prev.responseTime + (Math.random() - 0.5) * 10)
          ).toFixed(2)
        ),
      }));
      ...
      setInstances((prev) => {
        const avgCpu =
          prev.reduce((sum, inst) => sum + inst.cpuUsage, 0) / prev.length;
        ...
        setScalingPolicy((current) => {
          ...
          // Scaling logic
          if (
            avgCpu > current.targetCpuUtilization &&
            current.currentInstances < current.maxInstances
          ) {
            ...
            setTimeout(() => {
              if (prev.length < 3) {
                const newInstance: Instance = {
                  ...
                  status: "pending",
                  cpuUsage: 30,
                  httpEndpoint: `http://10.0.${
                    prev.length % 2 === 0 ? "10" : "11"
                  }.${100 + prev.length}:80`,
            ...
            newInstanceCount = Math.min(
              ...
              current.currentInstances + 1
            );
          } else if (
            avgCpu < current.targetCpuUtilization - 10 &&
            current.currentInstances > current.minInstances
          ) {
